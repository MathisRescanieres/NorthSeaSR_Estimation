...
for (f in files) {
  ...
  # Matrice dense base (depuis sparse.model.matrix -> as.matrix)
  X_full_train <- as.matrix(sparse.model.matrix(FORMULA_BASE, data = base_train))
  X_full_test  <- as.matrix(sparse.model.matrix(FORMULA_BASE, data = base_test))

  # Matrice dense température
  X_temp_train <- data.matrix(temp_train)
  X_temp_test  <- data.matrix(temp_test)

  rm(base_train, base_test, temp_train, temp_test); gc()

  # Concaténation dense
  X_train_full_temp <- cbind(X_full_train, X_temp_train)
  X_test_full_temp  <- cbind(X_full_test,  X_temp_test)
