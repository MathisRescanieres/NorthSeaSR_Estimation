...
setwd("C:\\Users\\Héloïse Calzan\\Documents\\travail_github\\Stage_M2_Calzan")
...
tableau <-readODS::read_csv(path = paste0(here::here(), "/SMALK_2022-01-06 11_47_25.csv"), na ="NA", col_names = TRUE, stringsAsFactors = FALSE)
```
...
```
Je vais faire la même chose qu'au dessus mais pour avoir le SR en fonction de l'année d'observation (comme ça on pourra comparer les deux)
```{r, fig.height=10, fig.width=10}
tableau_sp_year_FM <- aggregate(CANoAtLngt ~ Species + Year + Sex, data = tableau_donnees_filtrees, FUN = sum)
tableau_sp_year_FM <- tableau_sp_year_FM[order(tableau_sp_year_FM$Year),]

tableau_sp_year_F_M <- tableau_sp_year_FM %>%
  pivot_wider(names_from = Sex, values_from = CANoAtLngt)

tableau_sp_year_F_M$F[is.na(tableau_sp_year_F_M$F)] <- 0
tableau_sp_year_F_M$M[is.na(tableau_sp_year_F_M$M)] <- 0
#renomer les colonnes pour éviter les confusions avec FALSE
names(tableau_sp_year_F_M)[names(tableau_sp_year_F_M) == "F"] <- "Femelles"
names(tableau_sp_year_F_M)[names(tableau_sp_year_F_M) == "M"] <- "Males"
tableau_sp_year_SR <- mutate(tableau_sp_year_F_M, SR = Males/(Femelles + Males))
remove(tableau_sp_year_F_M, tableau_sp_year_FM)

unique(as.factor(tableau_sp_year_SR$Year))[which(!unique(as.factor(tableau_sp_year_SR$Year)) %in% unique(as.factor(tableau_sp_cohorte_2$Cohorte)))]
unique(as.factor(tableau_sp_cohorte_2$Cohorte))[which(!unique(as.factor(tableau_sp_cohorte_2$Cohorte)) %in% unique(as.factor(tableau_sp_year_SR$Year)))] 
unique(as.factor(tableau_sp_year_SR$Year)) %>% length()
unique(as.factor(tableau_sp_cohorte_2$Cohorte)) %>% length()

tableau_sp_year_SR$Year <- as.character(tableau_sp_year_SR$Year)
names(tableau_sp_year_SR)[which(names(tableau_sp_year_SR)== "Year")] <- "date"
names(tableau_sp_year_SR)[which(names(tableau_sp_year_SR)== "SR")] <- "SR_year"
names(tableau_sp_year_SR)[which(names(tableau_sp_year_SR)== "Femelles")] <- "Femelles_year"
names(tableau_sp_year_SR)[which(names(tableau_sp_year_SR)== "Males")] <- "Males_year"



tableau_sp_cohorte_year <- tableau_sp_cohorte_2
tableau_sp_cohorte_year$date <- as.character(tableau_sp_cohorte_year$Cohorte)
tableau_sp_cohorte_year <- tableau_sp_cohorte_year %>% relocate(date, .after = Cohorte)

tableau_sp_cohorte_year <- full_join(tableau_sp_cohorte_year, tableau_sp_year_SR, by = c("Species", "date"))



tableau_sp_cohorte_year$date <- as.numeric(tableau_sp_cohorte_year$date)

sp_final <- c("Clupea harengus", "Gadus morhua", "Melanogrammus aeglefinus", "Merlangius merlangus", "Microstomus kitt", "Pleuronectes platessa", "Pollachius virens", "Scomber scombrus", "Sprattus sprattus", "Trisopterus esmarkii")

#unique(tableau_sp_cohorte_year$Species)[which(!unique(tableau_sp_cohorte_year$Species) %in% sp_final)]
#sp_final[which(!sp_final %in% unique(tableau_sp_cohorte_year$Species))]

ggplot(tableau_sp_cohorte_year[which(tableau_sp_cohorte_year$Species %in% sp_final),], aes(x = date)) +
  geom_line(aes(y = SR), color= "hotpink4", alpha = 0.7, size = 1, na.rm = TRUE) +
  geom_point(aes(y = SR), color = "hotpink4", alpha = 0.7, size = 0.9, na.rm = TRUE) +
  geom_line(aes(y = SR_year), color = "darkseagreen4", alpha = 0.7, size = 1, na.rm = TRUE) +
  geom_point(aes(y = SR_year),  color = "darkseagreen4", alpha = 0.7, size = 0.9, na.rm = TRUE) +
  facet_wrap(~Species, ncol = 2, scales = "free") +
  theme_bw()+
  ggtitle("distribution du sex-ratio en fonction des cohorte")

ggplot(tableau_sp_cohorte_year[which(tableau_sp_cohorte_year$Species == "Clupea harengus"),], aes(x = date)) +
  geom_line(aes(y = SR), color= "hotpink4", alpha = 0.7, size = 1) +
  geom_point(aes(y = SR), color = "hotpink4", alpha = 0.7, size = 0.9) +
  geom_line(aes(y = SR_year), color = "darkseagreen4", alpha = 0.7, size = 1, na.rm = TRUE) +
  geom_point(aes(y = SR_year),  color = "darkseagreen4", alpha = 0.7, size = 0.9, na.rm = TRUE) +
  facet_wrap(~Species, ncol = 2, scales = "free") +
  theme_bw()+
...
#ggsave(filename = "SR_year_cohorte_Ch.png", width = 7, height = 5)

ggplot(tableau_sp_cohorte_year[which(tableau_sp_cohorte_year$Species == "Merlangius merlangus"),], aes(x = date)) +
  geom_line(aes(y = SR), color= "hotpink4", alpha = 0.7, size = 1) +
  geom_point(aes(y = SR), color = "hotpink4", alpha = 0.7, size = 0.9) +
  geom_line(aes(y = SR_year), color = "darkseagreen4", alpha = 0.7, size = 1, na.rm = TRUE) +
  geom_point(aes(y = SR_year),  color = "darkseagreen4", alpha = 0.7, size = 0.9, na.rm = TRUE) +
...
#ggsave(filename = "SR_year_cohorte_Mm.png", width = 7, height = 5)

ggplot(tableau_sp_cohorte_year[which(tableau_sp_cohorte_year$Species == "Trisopterus esmarkii"),], aes(x = date)) +
  geom_line(aes(y = SR), color= "hotpink4", alpha = 0.7, size = 1) +
  geom_point(aes(y = SR), color = "hotpink4", alpha = 0.7, size = 0.9) +
  geom_line(aes(y = SR_year), color = "darkseagreen4", alpha = 0.7, size = 1, na.rm = TRUE) +
  geom_point(aes(y = SR_year),  color = "darkseagreen4", alpha = 0.7, size = 0.9, na.rm = TRUE) +
...
#ggsave(filename = "SR_year_cohorte_Te.png", width = 7, height = 5)

remove(tableau_sp_year_SR)
```
