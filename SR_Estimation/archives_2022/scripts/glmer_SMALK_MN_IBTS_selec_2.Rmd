...
knitr::opts_knit$set(root.dir = file.path(proj_root, "SR_Estimation"))
knitr::opts_chunk$set(root.dir = file.path(proj_root, "SR_Estimation"))
```
...
```{r}
getwd()
table_cohortes <- read.csv("/home/mathis/NorthSeaSR_Estimation/SR_Estimation/archives_2022/data/table_cohorte_procedure_selection_2.csv", 
                           header = TRUE, sep = ",",  na ="NA", stringsAsFactors = FALSE)
table_donnees <- read.csv("/home/mathis/NorthSeaSR_Estimation/SR_Estimation/archives_2022/data/tableau_donnees_procedure_selection_2.csv", 
                          header = TRUE, sep = ",",  na ="NA", stringsAsFactors = FALSE)
```
...
```{r}
tableau_NAO <- read.csv("archives_2022/data/Monthly_mean_NAO_index_since_January_1950.csv", header = FALSE, sep = ";")
...
```{r}
tableau_AMO <- read.csv("archives_2022/data/AMO_unsmoothed_detrended_from_the_Kaplan_SST_V2.csv", header = FALSE, sep = ";")
...
```{r}
tableau_AMO_brute <- read.csv("archives_2022/data/AMO_non_detandence.csv", header = FALSE, sep = ";")
...
```{r}
tableau_GSNW <- read.csv("archives_2022/data/GSNW_month.csv", header = TRUE, sep = ";")
...
```{r}
table_SST_MN <- read.csv("archives_2022/data/table_SST_MN.csv", header = TRUE, sep = ";")
```
...
First, we need to get a range of dates common to all environmental data
sets.
...
Thus, the following pipeline will be calculated for dates between
`1966-01-01` and `2021-12-01`.
...
**Pipeline to find the common information between environmental
variables**
...
Calculation of the Spearman correlation. Pearson correlation should not
be used first as it only detects linear relationship between variables.

$$
\rho = 1 - \frac{6 \sum_{i=1}^{n} d_i^2}{n(n^2 - 1)}
$$ where:\
- $\rho$ : Spearman correlation coefficient\
- $d_i$ : difference between the ranks of the two variables for
observation $i$\
- $n$ : number of observations
...
> Since the variables are linearly correlated, **Principal Component
> Analysis (PCA)** is appropriate to explore the main patterns and
> reduce dimensionality while preserving linear variance.

> These investigations support the use of PCA to understand the joint
> variance of the environmental scores
...
p_mois_umap <- ggplot(umap_df, aes(x = UMAP1, y = UMAP2, color = Mois)) +
  ...
  theme_bw() +
  labs(title = "UMAP colorié par mois", color = "Mois")
...
p_annee_umap <- ggplot(umap_df, aes(x = UMAP1, y = UMAP2, color = Année)) +
  ...
  theme_bw() +
  labs(title = "UMAP colorié par année", color = "Année")
...
p_mois_tsne <- ggplot(tsne_df, aes(x = tSNE1, y = tSNE2, color = Mois)) +
  ...
  scale_color_manual(values = colorRampPalette(
    c("#2166ac","#74add1","#abd9e9","#e0f3f8",
      ...
      "#a50026","#762a83","#9970ab","#5aae61"))(12)) +
  theme_bw() +
  labs(title = "t-SNE colorié par mois", color = "Mois")
...
p_annee_tsne <- ggplot(tsne_df, aes(x = tSNE1, y = tSNE2, color = Année)) +
  ...
  scale_color_viridis_d(option = "plasma") +
  theme_bw() +
  labs(title = "t-SNE colorié par année", color = "Année")
...
**Aiglefin** Les trois premières cohortes sont à enlever
...
#fonctions Ensuite de cela, j'en ai fait des fonctions
...
En dessous c'est à peu près la même chose que j'ai fait au dessus
(permet de lancer les fonctions pour toutes les espèces à la fois)
...
hareng, Aiglefin, Tacaud M_i_A_40 M_i_H_40 M_i_L_40 M_i_Ma_40
M_i_Me_40_pta32 M_i_Mo_40_pta2 M_i_P_40_pta3 M_i_S_40_pt2
Mi_T_40_e_pta23 M_i_LS_40
...
Pour le Merlan et le tacaud, j'ai fais en plusieurs figures pour des
...
Hareng : taille, AMO et SST Aiglefin : taille, age, aMO et SST Lieu :
taille et âge LS : taille, NAO,GSNW Maquereau : taille et NAO Merlan :
tout Morue : rien Plie : taille, âge, AMO Sprat : taille, âge, AMO, GSNW
tacaud : tout

*figure rapport avec contribution des effets des variables cohorte,
fcohorte, âge, taille*
